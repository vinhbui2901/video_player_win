...
class WindowsVideoPlayer extends VideoPlayerPlatform {
  ...
  Future<int?> create(DataSource dataSource) async {
    if (dataSource.sourceType == DataSourceType.file) {
      ...
      var controller =
          WinVideoPlayerController.asset(dataSource.asset.toString());
      await controller.initialize();
      if (controller.textureId_ > 0) {
        mControllerMap[controller.textureId_] = controller;
        return controller.textureId_;
    ...
    else if(dataSource.sourceType == DataSourceType.contentUri){
       var controller =
          WinVideoPlayerController.contentUri(Uri.parse(dataSource.uri!));
      await controller.initialize();
      if (controller.textureId_ > 0) {
        mControllerMap[controller.textureId_] = controller;
        return controller.textureId_;
      ...
      throw UnimplementedError(
          'create() has not been implemented for dataSource type  [contentUri] in Windows OS');
    }
  }
